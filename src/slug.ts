...
export function packGlyphData(glyphs: SlugGlyph[]) {
  ...
  for (const g of glyphs) {
    ...
    for (const c of g.curves) {
      ...
      const x0 = i0 % TEX_WIDTH, y0 = (i0 / TEX_WIDTH) | 0;
      ...
      const x1 = i1 % TEX_WIDTH, y1 = (i1 / TEX_WIDTH) | 0;
  ...
  for (const [gi, g] of glyphs.entries()) {
    ...
    for (let i = 0; i < allBands.length; i++) {
      ...
      const tx = tl % TEX_WIDTH, ty = (tl / TEX_WIDTH) | 0;
...
export function prepareText(font: Font, text: string, fontSize: number) {
  ...
  const glyphDataMap = new Map<number, {
    glyph: SlugGlyph;
    glyphLocX: number;
    glyphLocY: number;
  }>();
